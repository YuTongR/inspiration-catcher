...
def search_materials(query: str) -> tuple[list[dict], str]:
    ...
    try:
        ...
        return [], "智能检索接口暂未接入，请稍后连接 B 模块。"
    except Exception as exc:
        return [], f"智能检索接口加载失败：{exc}"

    try:
        raw_results = agent_search_materials(keyword)
    except Exception as exc:
...
if page == "存入灵感":
    ...
    if search_clicked:
        st.session_state.search_results, st.session_state.search_error = search_materials(
            st.session_state.search_query
        )
    ...
    if st.session_state.search_error:
        ...
        st.markdown(
            '<div class="empty-shell">暂时没有可展示的检索结果。你可以稍后接入 B 模块接口后再试。</div>',
            unsafe_allow_html=True,
        )
    else:
    ...
    st.multiselect(
        ...
        default=st.session_state.draft_selected_ids,
